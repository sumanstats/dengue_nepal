...
dengue_gif <- data_longer %>%
  ...
  ggplot(aes(x = year, y = mean_cases, group=1)) +   geom_path(size = 1, color = "green") + theme(axis.text.x = element_text(angle = 90)) + facet_wrap(~Province, nrow = 3) + geom_area(alpha=0.4)+ scale_y_continuous(limits=c(0,320)) + transition_reveal(as.integer(year)) + labs(title = 'Dengue cases in Nepal by Province',x="Year (in B.S.)", y= "Mean cases", caption = "Source: Government of Nepal, EDCD Division")
...
animate(dengue_gif, height = 700, width =900)
